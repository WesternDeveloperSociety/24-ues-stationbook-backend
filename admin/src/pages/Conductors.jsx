...
function Conductors() {
    ...
    useEffect(() => {
        const fetchConductorRequests = async () => {
            try {
                const requests = await axios.get(`${import.meta.env.VITE_API_URL}/api/conductor/requests/all`);
        ...
        const fetchActiveConductors = async () => {
            try {
                const conductors = await axios.get(`${import.meta.env.VITE_API_URL}/api/conductor/get-active-conductors`);
